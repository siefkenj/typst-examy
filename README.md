...
Developed with Typst 0.15. A [dev container](.devcontainer) is provided for development.
...
```bash
# regenerate the API Reference section of this README:
# docs/generate-api.typ introspects the elembic element declarations (docs,
# argument types, defaults) and builds the reference as a Markdown string in
# #metadata(..) <api>, which this script extracts with
# `typst eval 'query(<api>)..'`, JSON-decodes, and splices between the
# API-DOCS-START/END markers above
./make_docs.sh
```
...
./make_dist.sh --tag=v0.3.0   # or: ./make_dist.sh --no-tag
```
...
3. Run `./make_dist.sh --tag=v0.3.0` (matching the tag from step 2). If the
